<?php
    ...
    class PhpTabBuilder extends AbstractTabBuilder
    {
        /**
         * @inheritDoc
         */
        public static function build(): string
        {
            ...
            return
                self::buildSection('PHP Environment', self::buildParametersHtml([
                    'PHP Version'        => self::escape(PHP_VERSION),
                    'PHP Version ID'     => (string) PHP_VERSION_ID,
                    'SAPI'               => self::escape(php_sapi_name() ?: 'N/A'),
                    'Memory Limit'       => self::escape(ini_get('memory_limit')),
                    'Max Exec Time'      => ini_get('max_execution_time') . 's',
                    'Display Errors'     => ini_get('display_errors') ? 'On' : 'Off',
                    'Error Reporting'    => self::escape(self::errorReportingToString((int) ini_get('error_reporting'))),
                    'Upload Max'         => self::escape(ini_get('upload_max_filesize')),
                    'Post Max Size'      => self::escape(ini_get('post_max_size')),
                    'Default Charset'    => self::escape(ini_get('default_charset') ?: 'UTF-8'),
                    'Error Log'          => self::escape(ini_get('error_log') ?: 'Default'),
                    'Log Errors'         => ini_get('log_errors') ? 'On' : 'Off',
                    'Timezone'           => self::escape(ini_get('date.timezone') ?: date_default_timezone_get()),
                ...
                self::buildSection('PHP Definitions', self::buildParametersHtml([
                    'ini File'            => self::escape(php_ini_loaded_file() ?: 'None'),
                    'Scanned ini Dirs'    => self::escape(php_ini_scanned_files() ?: 'None'),
                    ...
                    'PHP_OS_FAMILY'       => self::escape(PHP_OS_FAMILY),
                    'PHP_EXTRA_VERSION'   => self::escape(PHP_EXTRA_VERSION ?: 'None'),
                    'PHP_EOL'             => PHP_EOL === "\n" ? 'LF (\\n)' : (PHP_EOL === "\r\n" ? 'CRLF (\\r\\n)' : 'CR (\\r)'),
                    'PHP_PREFIX'          => self::escape(PHP_PREFIX),
                    'PHP_BINARY'          => self::escape(PHP_BINARY),
                    'PHP_SYSCONFDIR'      => self::escape(PHP_SYSCONFDIR),
                ])) .
        ...
        /**
         * Builds the section of the debug panel that lists all registered stream wrappers.
         *
         * @return string The HTML content for the stream wrappers section.
         */
        protected static function buildStreamWrappersSection(): string
        {
            ...
            return self::buildSection(
                ...
                . implode(', ', array_map(static fn($w) => self::escape($w) . '://', $wrappers))
                . '</span></div>'
            );
        }

        /**
         * Builds the section of the debug panel that displays key PHP security-related ini settings.
         *
         * @return string The HTML content for the PHP Security section.
         */
        protected static function buildSecurityIniSection(): string
        {
            ...
            return self::buildSection('PHP Security', self::buildParametersHtml([
        ...
        /**
         * Builds the section of the debug panel that lists all loaded PHP extensions and their versions.
         *
         * @return string The HTML content for the PHP Extensions section.
         */
        public static function buildPhpExtensionsHtml(): string
        {
            ...
            {
                $ver = phpversion($ext);
                $data[self::escape($ext)] = $ver ? self::escape($ver) : 'bundled';
            }
            return self::buildParametersHtml($data);
        }

        /**
         * Builds the section of the debug panel that lists all defined PHP constants, separated into user-defined and system-defined.
         *
         * @return string The HTML content for the PHP Constants section.
         */
        ...
        /**
         * Converts a PHP error_reporting level integer into a human-readable string representation.
         *
         * @param int $level The error_reporting level as an integer.
         *
         * @return string A string representation of the error_reporting level, listing all active error types.
         */
